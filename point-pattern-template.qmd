...
Control variance of spatial distribution for the density plot. Higher sigma shows vole density being a broader range. Too high and it looks like voles exist in the entire county.
...
```{r}
#| output: false
# output false is one way to make the output not get printed in a knitted or rendered document!
...
# y axis is G(r)
# x axis is r
# plot the observed value of G(r) as a black line
# plot the theoretical value of G(r) as a dashed red line

ggplot(gfunction_out)+
  geom_smooth(aes(r,obs),color="black")+
  geom_smooth(aes(r,theo),color="red",lty = 2)+
  geom_smooth(aes(r,lo),color="gray")+
  geom_smooth(aes(r,hi),color = "gray")+
  theme_minimal()+
  xlim(0, 5000)


gfunct <- gfunction_out %>% 
  as.data.frame() %>% 
  pivot_longer(cols = obs:hi, names_to = "model",values_to = "g_val")

ggplot(gfunct, aes(r,g_val))+
...
```{r}
r_vec_new <- seq(0,80000,5000)

lfunction_out <- envelope(voles_full, fun = Lest, r = r_vec_new, 
                          nsim = 100) 
```
